...
sys.path.append(os.path.join(data_home,'Claude-GPT-paper','dependent_functions'))
...
from Mopac_energy_charge_calculation import read_homo_lumo_energies
def get_energy_folder_path(data_home, overall_results_filename):
    ...
    xyz_folder_path = os.path.join(data_home, "Claude-GPT-paper", "pm7_charge_calculation", xyz_folder_name)
...
def get_charge_folder_path(data_home, overall_results_filename):
    ...
    # Combine with the data_home to form the full path
    charge_results_folder_path = os.path.join(data_home, "Claude-GPT-paper", "pm7_charge_calculation", charge_results_folder_name)
...
def process_and_analyze_homo_lumo_energy_data(data_home, overall_results_filename, allowed_keys=None, radius=2,start_letter='I'):
    # Generate the folder path for energy data
    folder_path = get_energy_folder_path(data_home, overall_results_filename)
    print("MOPAC Energy folder Path:", folder_path)
    
    # Read homo and lumo energy results
    homo_lumo_results = read_homo_lumo_energies(folder_path)
    
    # Process energy data, allowing for optional keys filtering
    energy_differences_deviation_by_prompt, energy_differences_filtered_prompt_mapping, energy_differences_smiles_latent_map = process_homo_lumo_energy_data(
        data_home, overall_results_filename, homo_lumo_results,start_letter=start_letter, radius=radius, allowed_keys=allowed_keys  
    )
